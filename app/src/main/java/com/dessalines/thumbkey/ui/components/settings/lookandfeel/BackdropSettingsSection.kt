...
fun BackdropSettingsSection(onChanged: () -> Unit) {
    ...
    Column(
        ...
        ChipRow(BackdropMode.entries, state.mode, { it.name.lowercase().replaceFirstChar(Char::uppercase) }) { mode ->
        ...
        if (state.mode != BackdropMode.NONE) {
            Box(Modifier.fillMaxWidth().height(54.dp)) { BackdropVisualLayer(state) }
        }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                listOf(BackdropPreset.BIRDIE_RAINBOW, BackdropPreset.SINEBOW).forEach { preset ->
                    FilterChip(
                        ...
                        onClick = {
                            val p = BackdropThemePreferences.stateForPreset(preset)
                            persist(state.copy(preset = preset, angleDegrees = p.angleDegrees, stops = p.stops))
                        },
                        label = { Text(if (preset == BackdropPreset.BIRDIE_RAINBOW) "Birdie Rainbow" else "Sinebow") },
                    )
                }
            }
            GradientEditor("Backdrop gradient", state.toBackdrop()) { gradient ->
                persist(state.copy(preset = BackdropPreset.CUSTOM, angleDegrees = gradient.angleDegrees, stops = gradient.stops))
            }
        }

        Text("Key surface", style = MaterialTheme.typography.titleMedium)
        ChipRow(KeySurfaceStyle.entries, keyTheme.surfaceStyle, { it.name.lowercase().replaceFirstChar(Char::uppercase) }) {
            persistKeys(keyTheme.copy(surfaceStyle = it))
        }
        when (keyTheme.surfaceStyle) {
            KeySurfaceStyle.GRADIENT -> GradientEditor("Key-space gradient", keyTheme.surfaceGradient) {
                persistKeys(keyTheme.copy(surfaceGradient = it))
            }
            KeySurfaceStyle.SOLID -> ColorEditor("Key color", keyTheme.surfaceColor) { persistKeys(keyTheme.copy(surfaceColor = it)) }
            KeySurfaceStyle.NONE -> Text("Transparent key faces; the backdrop shows through.")
        }

        Text("Key border", style = MaterialTheme.typography.titleMedium)
        ChipRow(KeyBorderStyle.entries, keyTheme.borderStyle, { it.name.lowercase().replaceFirstChar(Char::uppercase) }) {
            persistKeys(keyTheme.copy(borderStyle = it))
        }
        when (keyTheme.borderStyle) {
            KeyBorderStyle.GRADIENT -> GradientEditor("Border gradient", keyTheme.borderGradient) {
                persistKeys(keyTheme.copy(borderGradient = it))
            }
            KeyBorderStyle.SOLID -> ColorEditor("Border color", keyTheme.borderColor) { persistKeys(keyTheme.copy(borderColor = it)) }
            KeyBorderStyle.SHADOW -> {
                ColorEditor("Shadow color", keyTheme.shadowColor) { persistKeys(keyTheme.copy(shadowColor = it)) }
                Text("Shadow opacity: ${(keyTheme.shadowAlpha * 100).roundToInt()}%")
                Slider(keyTheme.shadowAlpha, { persistKeys(keyTheme.copy(shadowAlpha = it)) }, valueRange = 0f..1f)
                Text("Shadow elevation: ${keyTheme.shadowElevation.roundToInt()} dp")
                Slider(keyTheme.shadowElevation, { persistKeys(keyTheme.copy(shadowElevation = it)) }, valueRange = 0f..16f)
            }
        ...
        Text(fontName)
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(onClick = { fontPicker.launch(arrayOf("font/*", "application/x-font-ttf", "application/x-font-opentype", "application/octet-stream")) }) {
                Text("Import TTF / OTF")
            }
            Button(onClick = {
        ...
        Text("Imported fonts are copied into Keywi-managed storage, so they keep working if the original file moves.")

        Text("Suggestion motion", style = MaterialTheme.typography.titleMedium)
        ChipRow(SuggestionMotionStyle.entries, motionStyle, { it.name.lowercase().replaceFirstChar(Char::uppercase) }) {
            motionStyle = it
...
private fun GradientEditor(label: String, gradient: KeyboardBackdrop, onChange: (KeyboardBackdrop) -> Unit) {
    ...
    Slider(gradient.angleDegrees, { onChange(gradient.copy(angleDegrees = it)) }, valueRange = 0f..360f)
    ...
    if (expanded) {
        gradient.stops.forEachIndexed { index, stop ->
            Column(Modifier.fillMaxWidth().background(MaterialTheme.colorScheme.surfaceVariant).padding(8.dp)) {
                ...
                Slider(stop.position, { value ->
                    val next = gradient.stops.toMutableList().also { it[index] = stop.copy(position = value) }.sortedBy { it.position }
        ...
        Button(modifier = Modifier.fillMaxWidth(), onClick = {
            val position = if (gradient.stops.size < 2) 0.5f else gradient.stops.sortedBy { it.position }.zipWithNext()
                .maxByOrNull { it.second.position - it.first.position }?.let { (a, b) -> (a.position + b.position) / 2f } ?: 0.5f
            val color = gradient.stops.firstOrNull()?.color ?: Color.White
            onChange(gradient.copy(stops = (gradient.stops + KeyboardGradientStop(position, color)).sortedBy { it.position }))
        }) { Text("Add color stop") }
    }
}
...
@Composable
private fun Channel(label: String, value: Float, onChange: (Float) -> Unit) {
    Text("$label ${(value * 255).roundToInt()}")
    Slider(value, onChange, valueRange = 0f..1f)
}
