...
object KeyThemePreferences {
    ...
    private fun readGradient(prefs: android.content.SharedPreferences, prefix: String, fallback: KeyboardBackdrop): KeyboardBackdrop {
        val raw = prefs.getString("${prefix}_stops", null) ?: return fallback
        val stops = raw.split(";").mapNotNull { item ->
            val parts = item.split(":")
            if (parts.size != 2) null else runCatching {
                KeyboardGradientStop(parts[0].toFloat(), Color(parts[1].toULong()))
            }.getOrNull()
        }
        return if (stops.isEmpty()) fallback else KeyboardBackdrop(stops, prefs.getFloat("${prefix}_angle", fallback.angleDegrees))
    }

    private fun writeGradient(editor: android.content.SharedPreferences.Editor, prefix: String, gradient: KeyboardBackdrop) {
        editor.putFloat("${prefix}_angle", gradient.angleDegrees)
        editor.putString("${prefix}_stops", gradient.stops.joinToString(";") { "${it.position}:${it.color.value}" })
    }
